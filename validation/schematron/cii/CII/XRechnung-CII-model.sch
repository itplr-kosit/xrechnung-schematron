<pattern xmlns="http://purl.oclc.org/dsdl/schematron" is-a="model" id="CII-model">
    ...
    <param name="BR-DE-3" value="ram:CityName"/>
    <param name="BR-DE-4" value="ram:PostcodeCode"/>
    <param name="BR-DE-5" value="ram:PersonName"/>
    <param name="BR-DE-6" value="ram:TelephoneUniversalCommunication/ram:CompleteNumber"/>
    <param name="BR-DE-7" value="ram:EmailURIUniversalCommunication/ram:URIID"/>
    <param name="BR-DE-8" value="ram:CityName"/>
    <param name="BR-DE-9" value="ram:PostcodeCode"/>
    <param name="BR-DE-10" value="ram:CityName"/>
    <param name="BR-DE-11" value="ram:PostcodeCode"/>
    ...
    <param name="BR-DE-14" value="ram:RateApplicablePercent"/>
    <param name="BR-DE-15" value="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeAgreement/ram:BuyerReference"/>
    <param name="BR-DE-16" value="(rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeAgreement/ram:SellerTradeParty/ram:SpecifiedTaxRegistration/ram:ID[@schemeID='VA' or @schemeID='FC'], rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeAgreement/ram:SellerTaxRepresentativeTradeParty)"/>
    <param name="BR-DE-17" value="rsm:ExchangedDocument/ram:TypeCode = '326' or rsm:ExchangedDocument/ram:TypeCode = '380' or rsm:ExchangedDocument/ram:TypeCode = '384' or rsm:ExchangedDocument/ram:TypeCode = '381'"/>
    <param name="BR-DE-18" value="every $line in tokenize(rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:SpecifiedTradePaymentTerms/ram:Description,'\n\r?') satisfies if(count(tokenize($line,'#')) &gt; 1) then tokenize($line,'#')[1]='' and (tokenize($line,'#')[2]='SKONTO' or tokenize($line,'#')[2]='VERZUG') and string-length(replace(tokenize($line,'#')[3],'TAGE=[0-9]+',''))=0 and string-length(replace(tokenize($line,'#')[4],'PROZENT=[0-9]+\.[0-9]{2}',''))=0 and (tokenize($line,'#')[5]='' and empty(tokenize($line,'#')[6]) or string-length(replace(tokenize($line,'#')[5],'BASISBETRAG=[0-9]+\.[0-9]{2}',''))=0 and tokenize($line,'#')[6]='' and empty(tokenize($line,'#')[7])) else true()"/>
